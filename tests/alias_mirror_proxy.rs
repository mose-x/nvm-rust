...
fn alias_lookup_nonexistent_prints_not_found_but_exits_zero() {
    ...
    let s = stdout(&out);
    assert!(
        s.to_lowercase().contains("nope")
            && (s.to_lowercase().contains("not found")
                || s.to_lowercase().contains("does not exist")),
...
fn mirror_set_official_clears_mirror() {
    ...
    assert!(
        ...
        "mirror official should succeed: {}",
        stdout(&official)
    );
...
fn proxy_off_succeeds_and_writes_config() {
    ...
    assert!(
        s.to_lowercase().contains("off")
            || s.to_lowercase().contains("direct")
