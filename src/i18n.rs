...
impl<'de> Deserialize<'de> for Lang {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        // Fall back to English for an unknown/legacy lang code rather than
        // rejecting the whole config file.
        Ok(Lang::from_str(&s).unwrap_or(Lang::EN))
    }
}

/// Cached current language.
///
/// `T()` is called in tight loops (e.g. listing versions, rendering help),
/// and each call previously hit `load_config()` which reads config.json
/// from disk and parses JSON. We cache the resolved language here and
/// invalidate it whenever `set_language` writes a new value.
static LANG_CACHE: std::sync::OnceLock<std::sync::Mutex<Option<Lang>>> = std::sync::OnceLock::new();

fn lang_cache() -> &'static std::sync::Mutex<Option<Lang>> {
...
pub fn set_language(lang: Lang) -> Result<()> {
    ...
    save_config(&config)?;
    // Invalidate the cache so subsequent T() calls pick up the new value.
    if let Ok(mut guard) = lang_cache().lock() {
        *guard = None;
    }
    Ok(())
...
mod tests {
    ...
    fn test_unknown_key_returns_none_then_raw_key_via_t() {
        ...
        assert_eq!(
            t("definitely_not_a_real_key", Lang("cn")),
