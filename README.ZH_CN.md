...
| **中英文双语 `language en/cn`** | ✅（可插拔，放 `locales/*.toml` 即扩展） | ❌ | ❌ |
| **彩色输出** | ✅（colored） | ✅ | ❌ |
| **i18n 帮助文本** | ✅ | ❌ | ❌ |
...
切换显示语言。nvm-rs 内置英文和中文，并支持**零源码改动**添加新语言。

```bash
nvm language                      # 显示当前语言
nvm language cn                   # 切换到中文
nvm lang en                       # 切换到英文（别名）
nvm language zh                   # 中文别名
nvm lang en-us                    # 英文别名
```

设置会持久化到 `~/.nvm.rust/config.json`。

#### 添加新语言（约定优于配置）

往 `locales/` 目录里放一个 `xx.toml` 文件然后重新编译即可，**就这样**。
`build.rs` 会在编译期扫描 `locales/*.toml` 并自动注册新语言。不需要改
`i18n.rs`、`Cargo.toml` 或 `build.rs`。

1. **创建 locale 文件。** 把 `locales/en.toml` 复制为 `locales/xx.toml`
   （`xx` 是语言代码，比如 `jp`、`ca`、`de`），然后把所有 value 翻译过去。
   键集合必须和 `en.toml` 完全一致 —— `cargo test` 会强制检查（键不一致时
   `all_locales_have_same_keys_as_en` 测试会失败）。

2. **在文件末尾加 `[_meta]` 表**（可选但强烈建议）：

   ```toml
   [_meta]
   display_name = "日本語"            # 在 `nvm language` 列表里显示的名字
   aliases      = ["ja", "jpn"]       # `nvm language <别名>` 也能识别
   ```

   - `display_name` —— `nvm language` 输出里显示的人类可读名称
   - `aliases` —— 能解析到该语言的替代代码/名称

   如果不加 `_meta`，`display_name` 默认就是文件名 stem，也不会注册任何别名。

3. **重新编译** —— `cargo build`。新语言立即可用：
...
- 非英文 locale 里缺失的键会先 fallback 到英文值，再 fallback 到原始键名 ——
  所以部分翻译的 locale 永远不会显示乱码或残缺字符串。
- `en.toml` 是强制 baseline；缺了它 `build.rs` 会直接编译失败。
- 某个 locale 的 TOML 格式错误时，运行时会打印警告并跳过该语言，而不是
  让整个二进制崩溃。

#### 实现原理

- `build.rs` 在编译期解析每个 `locales/*.toml`，提取 `[_meta]` 表，生成
  `OUT_DIR/locales_generated.rs`，内含四个 `&'static` 数组：
  `LANG_CODES`、`LANG_DISPLAY_NAMES`、`LANG_ALIASES`、`LANG_STRINGS`
  （后者通过 `include_str!` 把每个 TOML 嵌进二进制）。
- `src/i18n.rs` 用 `include!` 引入这个生成文件，对外暴露 `Lang`、
  `from_str`、`T`、`format_t`、`available_lang_codes`。
- locale 文件在首次使用时解析成 `HashMap<String, String>`（lazy_static）；
  `[_meta]` 表会被过滤掉，绝不会作为可翻译字符串出现。
...
| `nvm language / lang [en\|cn\|...]` | 显示或设置显示语言（通过 `locales/*.toml` 可插拔） |
