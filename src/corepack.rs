...
pub fn corepack_disable(version: Option<&str>) -> anyhow::Result<()> {
    ...
    let output = Command::new(&corepack_path)
        .args(["disable", "--install-directory", &version_bin.display().to_string()])
        .env("PATH", &path_env)
        .output();
    ...
    if !success {
        for tool in &["pnpm", "pnpx", "yarn", "yarnpkg"] {
            ...
            if shim.exists() {
                // Only remove shims that look like corepack-managed files
                // (corepack writes tiny JS shims that require the corepack binary).
    ...
    if success {
        ...
        println!(
            "{} {} {}",
            "ℹ".cyan().bold(),
            T("corepack_maybe_not_enabled").cyan(),
            resolved.white().bold()
        );
    }
