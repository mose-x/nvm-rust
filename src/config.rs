...
pub fn remove_alias(name: &str) -> Result<()> {
    ...
    if aliases.aliases.remove(name).is_some() {
        ...
        println!(
            "{}",
            format_t("alias_removed", &[name.to_string()]).green()
        );
        Ok(())
    } else {
        anyhow::bail!(
            "{}",
            format_t("alias_not_found", &[name.to_string()])
        );
    }
}
...
fn bare_major_prefix(name: &str) -> Option<String> {
    ...
    // Must be all digits and at most one dot, i.e. "22" or "22.5".
    ...
    // Reject pure "." or "" after stripping.
...
pub fn handle_mirror(mirror: Option<&str>) -> Result<()> {
    ...
    match mirror {
        ...
        Some(url) => {
            ...
            println!(
                "{}",
                format_t("mirror_set", &[trimmed.to_string()]).green()
            );
        }
...
fn detect_shell_config() -> Option<String> {
    ...
    let candidates = vec![".zshrc", ".bashrc", ".bash_profile", ".profile", &fish_config];
    for c in candidates {
